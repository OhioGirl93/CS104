...
void DataBase::addProduct(Product* p)
{
	...
	for(set<string>::iterator it1 = prodKeys.begin(); it1!=prodKeys.end(); ++it1){
		if(keywords.find(*it1)==NULL){ //if it does not exist	
			set<Product*> s; //create a new set for the keyword
			s.insert(p);
			pair<string, set<Product*> > np ((*it1),s);
			keywords.insert(np);
		}
		else{ //if it does exist
			set<Product*> set = keywords.find(*it1)->getValue();
			set.insert(p);
			keywords.find(*it1)->setValue(set);
		}
	}
}
...
std::vector<Product*> DataBase::search(std::vector<std::string>& terms, int type)
{
	...
	if(type==0){ //if user is looking for the intersection of these terms
		...
		if(terms.size()>1){
			set1 = keywords.find(terms[0])->getValue();
		}
		else{
			if(keywords.find(terms[0])==NULL)
				return results;
			else{
				results.push_back(*(keywords.find(terms[0])->getValue().begin()));
				return results;
			}
		}

		for(unsigned int i=1; i<terms.size();i++){
			if(keywords.find(terms[i])!=NULL){
				set2 = keywords.find(terms[i])->getValue();
...
void DataBase::dump(std::ostream& ofile)
{
	...
	for(unsigned int i=0; i<products.size(); i++){
		for(unsigned int j=0; j<products[i]->getReviews().size(); j++){
			...
			products[i]->getReviews()[j]->username << " " <<
...
void DataBase::addReview(Review* r)
{
	...
	if(curr!=NULL){
		...
		for(unsigned int i=0; i<users.size(); i++){
			if(users[i]->getName()==uname){
				users[i]->addToReviewedList(curr);
				curr->addToReviewedUsers(users[i]);
			}
		}
	}
	else{
		cout << "Unable to parse review for: " << r->prodName << endl;
	}
}
